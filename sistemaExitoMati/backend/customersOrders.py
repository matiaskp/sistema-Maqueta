...
def get_customerOrders():
    ...
    try:
        ...
        customerOrders = [{"customer_id":row[0],"name": row[1], "lastName": row[2],"mail":row[3],"phone":row[4]} for row in rows]
        customerOrders2 = [{"id": row[0], "customer_id":row[1],"producto": row[4],"cantidad":row[3],"precio":row[5]} for row in rows2]
